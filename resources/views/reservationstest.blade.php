...
@section('content')
    ...
    <section class="bg-white p-8 rounded-lg shadow-lg">
        <div class="max-w-8xl mx-auto">
            ...
            <h2 class="text-4xl font-semibold mb-12 text-custom-font-blue ">Réservations à Venir</h2>
@php
    $hasUpcomingReservations = false;
@endphp

<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <!-- Colonne des réservations validées -->
    <div>
        <h3 class="text-2xl font-semibold mb-4 text-custom-font-blue">Réservations Validées</h3>
        @foreach ($reservations as $reservation)
            @if (Carbon\Carbon::parse($reservation->fin_date)->isFuture() && $reservation->status == 'approved')
                @php
                ...
                <div class="bg-gray-100 p-4 md:p-6 rounded-lg shadow mb-4 flex flex-col md:flex-row items-center">
                    ...
                    @endif
                    <div class="md:w-2/3">
                        <h3 class="text-2xl font-bold mb-2 text-custom-blue-header">{{ $reservation->carousel->name }}</h3>
                        ...
                        <p class="text-gray-700 mb-4 font-semibold">Date :
                            {{ \Carbon\Carbon::parse($reservation->debut_date)->format('d/m/Y') }} au
    ...
    <!-- Colonne des réservations en attente -->
    <div>
        <h3 class="text-2xl font-semibold mb-4 text-custom-font-blue">Réservations En Attente</h3>
        @foreach ($reservations as $reservation)
            @if (Carbon\Carbon::parse($reservation->fin_date)->isFuture() && $reservation->status == 'pending')
                @php
                    $hasUpcomingReservations = true;
                @endphp
                <div class="bg-gray-100 p-4 md:p-6 rounded-lg shadow mb-4 flex flex-col md:flex-row items-center">
                    ...
                    @endif
                    <div class="md:w-2/3">
                        <h3 class="text-2xl font-semibold mb-2 text-custom-blue-header">{{ $reservation->carousel->name }}</h3>
                        <div class="flex flex-col md:flex-row justify-between items-center mb-4">
                            <h3 class="text-xl font-semibold mb-2 md:mb-0">Réservation #{{ $reservation->id }}</h3>
                            <div class="bg-orange-500 text-white rounded-md p-2">
                                En attente
                            </div>
                        </div>
                        <p class="text-gray-700 mb-4 font-semibold">Date :
...
<h2 class="text-4xl font-semibold mb-12 mt-8 text-custom-font-blue">Historique des Réservations</h2>
@php
...
<div>
    <h3 class="text-2xl font-semibold mb-4 text-custom-font-blue">Réservations Validées</h3>
    @foreach ($reservations as $reservation)
        @if (Carbon\Carbon::parse($reservation->fin_date)->isPast() && $reservation->status == 'approved')
            @php
...
<div>
    <h3 class="text-2xl font-semibold mb-4 text-custom-font-blue">Réservations En Attente</h3>
    @foreach ($reservations as $reservation)
        @if (Carbon\Carbon::parse($reservation->fin_date)->isPast() && $reservation->status == 'pending')
            @php
                $hasPastReservations = true;
            @endphp
            <div class="bg-gray-100 p-4 md:p-6 rounded-lg shadow mb-4 flex flex-col md:flex-row items-center">
