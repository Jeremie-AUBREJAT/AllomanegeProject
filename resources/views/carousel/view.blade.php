...
@section('content')
    ...
    <div class="flex justify-center m-8">
        <a href="/carousel/create"
            class="block mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Créer
            un Manège</a>
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            @foreach ($carousels as $carousel)
                <div class="bg-white rounded-lg shadow-md p-4">
                    ...
                    <p class="text-custom-blue-header font-semibold text-lg mb-2 mt-4"name="price">{{ $carousel->price }} €
                    </p>
                    <div class="flex flex-wrap justify-between mt-auto space-y-4 sm:space-y-0">
                        <div class="flex space-x-4">
                            <a href="/carousel/update/{{ $carousel->id }}"
                                class="bg-green-500 hover:bg-green-700 active:bg-green-900 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Modifier</a>
                            <a href="/carousel/update/{{ $carousel->id }}"
                                class="font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline 
                                    @if ($carousel->status == 'pending') bg-orange-500 hover:bg-orange-700 active:bg-orange-900 text-white 
                                    @elseif($carousel->status == 'approved') 
                                        bg-blue-500 hover:bg-blue-700 active:bg-blue-900 text-white @endif">
                            ...
                            <a href="/carousel/{{ $carousel->id }}/reservations"
                                class="bg-blue-600 hover:bg-blue-700 active:bg-blue-900 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Réservations</a>
                        </div>
                    </div>
                </div>
            @endforeach
