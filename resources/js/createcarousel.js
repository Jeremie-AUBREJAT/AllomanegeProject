...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkValidity = () => {
        let isValid = true;
        const fileInputs = document.querySelectorAll('input[name="imageCreate[]"]');
        ...
        const imageCreateValidator = validations['imageCreate'];
        const filesToValidate = Array.from(fileInputs).reduce((acc, input) => acc.concat(Array.from(input.files)), []);
        const isValidImages = imageCreateValidator.validate(filesToValidate);
        const imageCreateError = document.getElementById('imageCreateError');

        if (!isValidImages && totalFiles > 0) {
            isValid = false;
            if (imageCreateError) {
                imageCreateError.textContent = imageCreateValidator.message;
            }
            fileInputs.forEach(input => {
                input.classList.add('border-red-500');
            });
            ...
            isValid = false;
            if (imageCreateError) {
                imageCreateError.textContent = "Vous devez sélectionner au moins une image.";
            }
            fileInputs.forEach(input => {
                input.classList.add('border-red-500');
            });
